...
class Training:
    ...
    def train_test_val_split(self):
        ...
        spk3_files = self.get_files('')  # Add your dataset path here (Containing mixture of speaker 3)
        ...
        train_spk3_files = spk3_files[:3300]
        val_spk3_files = spk3_files[3300:3500]
        test_spk3_files = spk3_files[3500:]
        ...
        self.train_spk3 = self.load_preprocess_audio(train_spk3_files)
        self.test_spk3 = self.load_preprocess_audio(test_spk3_files)
        self.val_spk3 = self.load_preprocess_audio(val_spk3_files)

    def get_dataloaders(self):
        train_dataset = torch.utils.data.TensorDataset(self.train_mix, self.train_spk1, self.train_spk2, self.train_spk3)
        val_dataset = torch.utils.data.TensorDataset(self.val_mix, self.val_spk1, self.val_spk2, self.val_spk3)
        test_dataset = torch.utils.data.TensorDataset(self.test_mix, self.test_spk1, self.test_spk2, self.test_spk3)
    ...
    def train_model(self):
        ...
        for epoch in range(start_epoch, 100):
            self.model.train()
            epoch_loss = 0
            for batch_idx, (w, x, y ,z) in enumerate(train_loader):
                waveform = w.to(self.device)
                spk1 = x.to(self.device)
                spk2 = y.to(self.device)
                spk3 = z.to(self.device)
                
                opt.zero_grad() 
                
                with torch.cuda.amp.autocast():  
                    output = self.model(waveform)
                    loss_value = criterion(output[0], spk1) + criterion(output[1], spk2) + criterion(output[2], spk3)
